...
from .shared_hooks import Grid, SharedTorchLmHooks


class SharedTorchLmInference:
    """Shared turbo DFS, cell-prob inference, and candidate scoring (decoding policy stays in backends)."""

    _loaded: bool
    _torch: Any
    _model: Any
    _tokenizer: Any
    _torch_lm_hooks: SharedTorchLmHooks
    ...
    def turbo_dfs_beams(
        ...
        *,
        ...
        hooks = self._torch_lm_hooks
        fmt = hooks.make_formatter(self._tokenizer)
        arc_tokens, pad_id, eos_id = resolve_turbo_token_table(self._tokenizer, str(fmt.im_end))
        wall = (
            float(inner_loop_wall_sec)
            if inner_loop_wall_sec is not None
            else float(hooks.reference_inner_loop_wall_sec)
        )
        raw = hooks.inference_turbo_dfs(
            self._model,
            prefix_token_ids,
            int(max_new_tokens),
            float(max_score),
    ...
    def infer_cell_probs(self, input_grid: Grid) -> list[list[list[float]]]:
        ...
        formatter = hooks.make_formatter(tokenizer)
    ...
    def score_candidate_grid(self, task_payload: dict[str, Any], candidate_grid: Grid) -> float:
        ...
        answer = hooks.arc_grid_to_text_lines(candidate_grid)
        qtxt = ""
