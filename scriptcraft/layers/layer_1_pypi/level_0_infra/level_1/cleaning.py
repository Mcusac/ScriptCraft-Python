...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Apply light, general-purpose cleaning steps to a DataFrame before saving.

    This is intentionally conservative: it focuses on removing common CSV artifacts
    (like `Unnamed:` columns), dropping fully-empty columns, and normalizing missing
    values to empty strings so output files are consistent.
    """
    if df is None:
    ...
    # Remove common CSV artifacts like "Unnamed: 0"
    cleaned = cleaned.loc[:, ~cleaned.columns.astype(str).str.contains("^Unnamed", na=False)]

    # Drop columns that are completely empty
    cleaned = cleaned.dropna(axis=1, how="all")

    # Normalize missing values for downstream CSV export
