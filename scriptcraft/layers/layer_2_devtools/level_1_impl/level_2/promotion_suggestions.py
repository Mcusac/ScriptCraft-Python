"""
Promotion suggestions (report-only).
"""
...
from scriptcraft.layers.layer_2_devtools.level_1_impl.level_1.api_audit import (
...
def main() -> None:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit with code 1 if any promotion candidates are suggested.",
    )
    parser.add_argument(
        "--min-total-inbound",
        type=int,
    ...
    if args.strict:
        promo_ok = int(
            (data.get("payload", {}).get("counts", {}).get("promotion", {}) or {}).get("ok", 0)
        )
        if promo_ok > 0:
            raise SystemExit(1)
