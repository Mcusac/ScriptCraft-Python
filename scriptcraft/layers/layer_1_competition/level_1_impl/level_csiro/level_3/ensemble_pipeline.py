...
def ensemble_pipeline(
    ...
    **kwargs
    ...
    _logger.info(f"Ensembling top {len(top_results)} models:")
    for i, result in enumerate(top_results, 1):
        _logger.info(f"  {i}. {result.get('variant_id', 'unknown')}: CV={result.get('cv_score', 0):.4f}")
    ...
    _logger.info("Running regression ensemble pipeline...")
    ...
    _logger.info("✅ Ensemble pipeline complete!")
...
def ensemble_pipeline_from_paths(
    ...
    **kwargs
    ...
    _logger.info(f"Ensembling {len(model_paths)} models from paths...")
    submission_df = regression_ensemble_pipeline(
        data_root=data_root,
        ensemble_config=ensemble_config,
        dataset_type='split',
    ...
    _logger.info("✅ Ensemble from paths pipeline complete!")
