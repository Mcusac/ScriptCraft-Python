...
from scriptcraft.layers.layer_0_core.level_0 import ensure_dir, is_kaggle
...
def find_project_input_root(from_file: str) -> Optional[Path]:
    """
    Find project root that contains an `input/` directory (local mimic layout).

    This is a public wrapper around the internal local-mimic discovery logic so
    higher layers can reuse it without importing private helpers.
    """
    return _find_project_input_root(from_file)
