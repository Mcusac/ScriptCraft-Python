...
def _find_workspace_root(start: Path) -> Path:
    """
    Find the ScriptCraft workspace root by walking up until we find the
    `implementations/python/python-package` folder.
    """
    current = start.resolve()
    for parent in [current, *current.parents]:
        candidate = parent / "implementations" / "python" / "python-package"
        if candidate.exists():
            return parent
    # Fallback to the previous behavior (best effort)
    return start.parent.parent


# Centralized path resolution (robust to being run from nested layers)
WORKSPACE_ROOT = _find_workspace_root(Path(__file__).parent)
PACKAGE_ROOT = WORKSPACE_ROOT / "implementations" / "python" / "python-package"
